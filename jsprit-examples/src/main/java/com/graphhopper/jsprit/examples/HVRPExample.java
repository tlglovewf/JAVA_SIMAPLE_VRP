...
public class HVRPExample {
    ...
    public static void main(String[] args) {
        ...
        //add vehicle - finite fleet   dimension stand for size
        VehicleType type1 = VehicleTypeImpl.Builder.newInstance("type_1").addCapacityDimension(0, 120).setCostPerDistance(1.0).build();
        VehicleImpl vehicle1 = VehicleImpl.Builder.newInstance("1_1").setStartLocation(Location.newInstance(0, 0)).setType(type1).build();
        ...
        try
        {
            File file = new File("/media/tu/Work/Sources/jsprit/route.txt");
            if(file.isFile() && file.exists())
            {
                ...
                while((line = bufreader.readLine()) != null)
                {
                    ...
                    vrpBuilder.addJob(Service.Builder.newInstance(String.valueOf(index++)).addSizeDimension(0, 3).setLocation(Location.newInstance(x, y)).build());
                }
            }
            else
            {
